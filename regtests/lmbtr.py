...
class LMBTRTests(TestBaseClass, unittest.TestCase):
    ...
    # def test_flatten(self):
        ...
        # # K2 unflattened
        # desc = LMBTR([1, 8], k=[2], grid={"k2": {"n": n, "min": 0, "max": 2, "sigma": 0.1}}, periodic=False, flatten=False, sparse=False)
    ...
    # def test_sparse(self):
        ...
        # """
        # # Dense
        # desc = LMBTR([1, 8], k=[1], grid=default_grid, periodic=False, flatten=True, sparse=False)
        # vec = desc.create(H2O, positions=[0])[0]
        # self.assertTrue(type(vec) == np.ndarray)

        # # Sparse
        # desc = LMBTR([1, 8], k=[1], grid=default_grid, periodic=False, flatten=True, sparse=True)
        # vec = desc.create(H2O, positions=[0])[0]
    ...
    def test_k2_weights_and_geoms_finite(self):
        ...
        pos = H2O.get_positions()
        assumed_geoms = {
            (1, 2): 2*[1/np.linalg.norm(pos[0] - pos[1])],
        }
        self.dict_comparison(assumed_geoms, geoms)
    ...
    # def test_k2_weights_and_geoms_finite_virtual(self):
        # """Tests that the values of the weight and geometry functions are
        # correct for the k=2 term.
        # """
        # lmbtr = LMBTR([1, 8], k=[2], grid=default_grid, virtual_positions=True, periodic=False)
        # lmbtr.create(H2O, positions=[[0.5, 0.5, 0.5]])
        # geoms = lmbtr._k2_geoms
        # weights = lmbtr._k2_weights
        # print(geoms)
        # print(weights)
