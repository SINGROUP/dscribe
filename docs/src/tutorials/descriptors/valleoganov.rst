...
=======================
Shortcut for implementing the fingerprint descriptor by Valle and Oganov :cite:`valle2010crystal`
for :math:`k=2` and :math:`k=3` :cite:`bisbo2020global` using MBTR. It is a subclass of MBTR that
automatically uses the right weightings and normalizes the output. The descriptor
can also be accessed in MBTR by setting the right parameters: see the :doc:`MBTR tutorial <mbtr>`.

It is advisable to first check out
the :doc:`MBTR tutorial <mbtr>` to understand the basics of the MBTR framework.
The differences compared to MBTR are:

  - The :math:`k=1` term has been removed.
  - Rather than a cutoff weight, both weightigs use a cutoff distance *r_cutoff*.
  - In the grid setup, *min* is always 0 and *max* has the same value as *r_cutoff*.
  - Normalization and weigtings are automatically set to their Valle-Oganov options.
