...
cdef extern from "cmbtr.h":
  cdef cppclass CMBTR:
        ...
        pair[map[string,vector[float]], map[string,vector[float]]] getK1MapCython(string, string, map[string, float]) except +
        pair[map[string,vector[float]], map[string,vector[float]]] getK2MapCython(string, string, map[string, float]) except +
