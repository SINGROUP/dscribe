...
# F_train = torch.Tensor(F_train).cuda()
# F_valid = torch.Tensor(F_valid).cuda()
# dD_dr_train = torch.Tensor(dD_dr_train).cuda()
# dD_dr_valid = torch.Tensor(dD_dr_valid).cuda()
if cuda:
...
class FFNet(torch.nn.Module):
    ...
    def __init__(self, n_features, n_hidden, n_out):
        ...
        torch.nn.init.normal_(self.linear2.weight, mean=10, std=10.0)
...
def energy_force_loss(E_pred, E_train):
...
for i_epoch in range(n_max_epochs):
    ...
    for i in range(0, D_train.size()[0], batch_size):
        ...
        optimizer.zero_grad()
        loss = energy_force_loss(E_train_pred_batch, E_train_batch)
    ...
    valid_loss = energy_force_loss(E_valid_pred, E_valid)
...
E_whole_pred = E_whole_pred.detach().numpy()[0:n_train]
if cuda:
    E_whole = E_whole.cpu()[0:n_train]
E_whole = E_whole.detach().numpy()
# order = np.argsort(r_whole)
fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(10, 10))
ax1.plot(np.arange(n_train),  E_whole, label="True", linewidth=3, linestyle="-")
ax1.plot(np.arange(n_train), E_whole_pred, label="Predicted", linewidth=3, linestyle="-")
