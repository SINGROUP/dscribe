...
class SOAP(Descriptor):
    ...
    def __init__(
        ...
        supported_rbf = {"gto", "polynomial", "gto_general"}
 ...
 #=====================================================================================   
 #=====================================================================================   
    def get_basis_GTO_general(self, rcut, nmax, lmax):

        alphas, betas =  get_basis_gto(self, rcut, nmax, lmax)
...
#                fs[n - 1, :] = (rcut - np.clip(rx, 0, rcut)) ** (n + 2)
                fs[n, :] = rx**l * np.exp(-alphas[ l, n ] * rx**2)
            gss = np.dot(betas[l,:,:], fs)
            gsss.insert(0,gss)

        return rx, np.concatenate(gsss, axis=0)
