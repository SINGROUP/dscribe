...
void getC(double* C, double* ws, double* rw2, double * gss, double* summed, double rCut, int lMax, int rsize, int gnsize, int nCenters, int nNeighbours, double eta, double* weights)
{
    // Initialize to zero
    ...
    for (int n = 0; n < gnsize; n++) {
        ...
        if (nCenters > 0) {
            ...
            for (int iCenter = 0; iCenter < nCenters; ++iCenter) {
                for (int rw = 0; rw < rsize; rw++) {
                    C[2*(lMax+1)*(lMax+1)*n] += weight * 0.5*0.564189583547756*rw2[rw]*ws[rw]*gss[rsize*n + rw]*exp(-eta*rw2[rw]);
                }
            }
        }
        for (int l = 0; l < lMax+1; l++) {
...
void getCl(double* C, double* ws, double* rw2, double * gnl, double* summed, double rCut, int lMaxPlus100, int rsize, int gnsize, int nCenters, int nNeighbours, double eta, double* weights)
{
    // Initialize to zero
    int lMax = lMaxPlus100 - 100;
    memset(C, 0.0, 2*(lMax+1)*(lMax+1)*gnsize*sizeof(double));
...
if(lMax>99)
    ...
    free(Cs); free(cf); free(dx); free(dy); free(dz); free(ris); free(oOri); free(ws); free(oOr); free(rw2) ; free(oO4arri); free(minExp); free(pluExp); free(C);
}
